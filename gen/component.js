...
const { root, checkDirExist, firstUpperCase } = require('../util/index')
...
module.exports = function genComponent(params) {
    ...
    if (checkDirExist(componentRootPath) && !checkDirExist(componentPath)) {
        ...
        // 替换模块名
        const newIndexText = indexText.replace(/Name/g, firstUpperCase(params.name));
        ...
        if (params.storybook) {
            ...
            // 替换模块名
            const newStoryText = storyText.replace(/Name/g, firstUpperCase(params.name));
